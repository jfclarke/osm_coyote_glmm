...
# about
This is a data formatting code walk-through for the manuscript *Energy infrastructure clears the way for coyotes in Alberta's oil sands*. It was written by Jamie F Clarke with help from Marissa A Dyck, and based on preliminary scripts by Larissa Bron. The data required to run the script is available on [Jamie's GitHub](https://github.com/jfclarke/osm_coyote_glmm). Happy formatting :-)
...
load in total detections of other mammal species and join the 2021-2022 + 2022-2023 datasets using `purr`:
...
join dataframes together into `coyote_data` master dataframe and clean it up:
...
check histograms for each covariate of interest - looking for very left-skewed, zero-inflated plots that indicate little presence on the lansdscape (for modelling):
```{r hist, fig.dim=c(8,4.5)}
...
the plots above show us that there are too few detections of cougars and caribou to take further into analyses - so remove cougars and caribou from the dataframe:
...
# save formatted project dataframe
...
make a new variable of all the covariates to consider:
...
**side note:** the text size in the correlation plots is really tiny... to change that, run the following line of code and, in the pop-up window, on line 17, change `cex` to 5 (or whatever you want):
```{r cex, eval=FALSE}
...
```{r pearson, warning=FALSE, fig.dim=c(8,8)}
...
```{r spearman, warning=FALSE, fig.dim=c(8,8)}
chart.Correlation(coyote_cor,
                  histogram = TRUE,
                  method = 'spearman')
```
